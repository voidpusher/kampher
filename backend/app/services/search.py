...
class SearchService:
    ...
    async def search_posts(
        ...
        if mode == "keyword":
            ids = await self._keyword(query, limit, source)
            return [SearchHit(pid, 1.0 / (i + 1), "keyword") for i, pid in enumerate(ids)]
        if mode == "semantic":
            ids = await self._semantic(query, limit, source, industry_slug)
            return [SearchHit(pid, 1.0 / (i + 1), "semantic") for i, pid in enumerate(ids)]

        keyword_ids, semantic_ids = await asyncio.gather(
            self._keyword(query, limit * 2, source),
            self._semantic(query, limit * 2, source, industry_slug),
        )
    ...
    async def _keyword(self, query: str, limit: int, source: str | None) -> list[uuid.UUID]:
    ...
    async def _semantic(
        self, query: str, limit: int, source: str | None, industry_slug: str | None
    ) -> list[uuid.UUID]:
